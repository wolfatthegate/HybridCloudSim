...
class AMDRyzen(CPU):
    ...
    def process_job(self, job, wait_time_start):
        ...
        # --- Quantum/Parameterized Workload Calculation ---
        num_qubits = getattr(job, "num_qubits", None)
        depth = getattr(job, "depth", None)
        param_count = getattr(job, "param_count", None)

        # Check if quantum parameters are available
        if num_qubits is not None and depth is not None:
            # Complexity scales exponentially with qubits: 2^n * depth
            # Param count multiplier accounts for optimization loops/gradients
            p_count = param_count if param_count is not None else 1
            
            # Calibration scaling factor to convert algorithmic complexity to "cpu_work" units
            scaling_factor = 1.5 * 1e-6 
            
            work = (2 ** num_qubits) * depth * p_count * scaling_factor
        else:
            # Fallback to standard classical work attribute
        ...
        # Log quantum specific parameters if they exist
        if num_qubits is not None:
            self.job_records_manager.log_job_event(job_id, 'num_qubits', num_qubits)
            self.job_records_manager.log_job_event(job_id, 'circuit_depth', depth)
            self.job_records_manager.log_job_event(job_id, 'param_count', param_count)
